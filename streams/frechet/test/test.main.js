...
var frechet = require( './../../../base/frechet' ).factory;
...
tape( 'the function throws an error if provided an options argument which is not an object', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			randomStream( 2.0, 5.0, 3.0, value );
		};
	}
});
...
tape( 'the function is a constructor which returns a readable stream', function test( t ) {
	...
	s = new RandomStream( 2.0, 5.0, 3.0 );
...
tape( 'the constructor returns a readable stream (no new)', function test( t ) {
	var s = randomStream( 2.0, 5.0, 3.0 );
	t.equal( s instanceof Readable, true, 'returns expected value' );
	t.end();
});
...
tape( 'the constructor returns a stream for generating pseudorandom numbers from an frechet distribution', function test( t ) {
	var iStream;
	var result;
	var rand;
	var opts;
...
tape( 'the constructor returns a stream for generating pseudorandom numbers from an frechet distribution (object mode)', function test( t ) {
	var iStream;
	var count;
	var rand;
	var opts;
...
tape( 'the constructor supports returning a seeded readable stream', function test( t ) {
	...
	s1 = randomStream( 2.0, 5.0, 3.0, opts );
	s1.on( 'end', onEnd1 );

	s2 = randomStream( 2.0, 5.0, 3.0, opts );
	s2.on( 'end', onEnd2 );
