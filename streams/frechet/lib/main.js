...
function RandomStream( alpha, s, m, options ) {
	...
	if ( !isPositiveNumber( alpha ) ) {
		throw new TypeError( format( 'invalid argument. First argument must be a positive number primitive and not `NaN`. Value: `%s`.', alpha ) );
	}
	if ( !isPositiveNumber( s ) ) {
		throw new TypeError( format( 'invalid argument. Second argument must be a positive number primitive and not `NaN`. Value: `%s`.', s ) );
	}
